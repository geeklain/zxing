...
import UPCEANReader, {L_AND_G_PATTERNS, MIDDLE_PATTERN, L_PATTERNS} from './UPCEANReader';
...
//                0    1    1   0   0    1   == 0x19
//
export const FIRST_DIGIT_ENCODINGS = Object.freeze([0x00, 0x0B, 0x0D, 0xE, 0x13, 0x19, 0x1C, 0x15, 0x16, 0x1A]);
...
export default class EAN13Reader extends UPCEANReader {
  ...
  decodeMiddle(row, startRange, resultString) {
    ...
    for (let x = 0; x < 6 && rowOffset < end; x++) {
      const bestMatch = EAN13Reader.decodeDigit(row, counters, rowOffset, L_AND_G_PATTERNS);
    ...
    const middleRange = EAN13Reader.findGuardPattern(row, rowOffset, true, MIDDLE_PATTERN);
    rowOffset = middleRange[1];

    for (let x = 0; x < 6 && rowOffset < end; x++) {
      const bestMatch = EAN13Reader.decodeDigit(row, counters, rowOffset, L_PATTERNS);
      resultString.push(bestMatch);
      counters.forEach(function(counter) {
        rowOffset += counter;
      });
    }
